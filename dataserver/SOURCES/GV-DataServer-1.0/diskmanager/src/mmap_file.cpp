...
namespace tfs
{
  namespace dataserver
  {
    ...
    MMapFile::~MMapFile()
    {
      if (data_)
      {
        printf("mmap file destruct, fd: %d maped size: %d, data: %p", fd_, size_, data_);
    ...
    bool MMapFile::map_file(const bool write)
    {
      ...
      {
        printf("ensure file size failed in mremap. size: %d", size_);
        return false;
      }
      ...
      if (MAP_FAILED == data_)
      {
        printf("map file failed: %s", strerror(errno));
        size_ = 0;
        fd_ = -1;
        data_ = NULL;
        return false;
      ...
      printf("mmap file successed, fd: %d maped size: %d, data: %p", fd_, size_, data_);
    ...
    {
      printf("map file need remap. size: %d, fd: %d", size_, fd_);
      if (fd_ < 0 || data_ == NULL)
      {
        printf("mremap not mapped yet");
        return false;
      }

      if (size_ == mmap_file_option_.max_mmap_size_)
      {
        printf("already mapped max size, now size: %d, max size: %d", size_, mmap_file_option_.max_mmap_size_);
        return false;
      }
      ...
      {
        printf("ensure file size failed in mremap. new size: %d", new_size);
        return false;
      }

      printf("mremap start. fd: %d, now size: %d, new size: %d, old data: %p", fd_, size_, new_size, data_);
      void* new_map_data = mremap(data_, size_, new_size, MREMAP_MAYMOVE);
      if (MAP_FAILED == new_map_data)
      {
        printf("ensure file size failed in mremap. new size: %d, error desc: %s", new_size, strerror(errno));
        return false;
      }
      else
      {
        printf("mremap success. fd: %d, now size: %d, new size: %d, old data: %p, new data: %p", fd_, size_, new_size,
            data_, new_map_data);
      }
    ...
    bool MMapFile::ensure_file_size(const int32_t size)
    {
      ...
      if (fstat(fd_, &s) < 0)
      {
        printf("fstat error, error desc: %s", strerror(errno));
        return false;
      }
      if (s.st_size < size)
      {
        ...
        {
          printf("ftruncate error, size: %d, error desc: %s", size, strerror(errno));
          return false;
        }
      }
