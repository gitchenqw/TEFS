...
namespace tfs
{
  namespace common
  {
    ...
    int Func::assign_ip_addr(const char* ifname, const char* ip, const char* mask)
    {
      ...
      {
        close(fd);
        TBSYS_LOG(TFERROR, "SIOCSIFADDR: %s", strerror(errno));
        return EXIT_IOCTL_ERROR;
      }
      if (ioctl(fd, SIOCGIFFLAGS, &ifr) < 0)
      {
        close(fd);
        TBSYS_LOG(TFERROR, "SIOCGIFFLAGS: %s", strerror(errno));
        return EXIT_IOCTL_ERROR;
      }
      ...
      {
        close(fd);
        TBSYS_LOG(TFERROR, "SIOCSIFFLAGS: %s", strerror(errno));
        return EXIT_IOCTL_ERROR;
      }
      if (mask != NULL)
      {
        ...
        {
          close(fd);
          TBSYS_LOG(TFERROR, "SIOCSIFNETMASK: %s", strerror(errno));
          return EXIT_IOCTL_ERROR;
        }
      }
      return TFS_SUCCESS;
    ...
    char* Func::safe_malloc(const int32_t len, char* data)
    {
      ...
      {
        TBSYS_LOG(TFERROR, "allocate to large memory: len: %u > maxlen: %u", len, TFS_MALLOC_MAX_SIZE);
        return NULL;
      }
      if (data == NULL)
      {
        ...
        const bool char_type /*= true*/, const int32_t log_level /*= TBSYS_LOG_LEVEL_DEBUG*/)
    {
      ...
      for(n = 1; n <= size; n++) 
      {
        ...
        { 
          /* line completed */
          if (char_type) 
            TBSYS_LOG(TFINFO,"[%4.4s]   %-50.50s  %s\n", 
                addrstr, hexstr, charstr);
          else 
            TBSYS_LOG(TFINFO, "[%4.4s]   %-50.50s\n", 
                addrstr, hexstr);
          hexstr[0] = 0;
          charstr[0] = 0;
        } 
