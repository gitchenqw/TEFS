...
UInt32   TCPListenerSocket::TotalConnectNum = 0;
...
#ifdef _EPOLL
                //this->RequestEvent(EPOLLIN|EPOLLET);
#else   
...
//test acceptError = ENOENT;
        ...
        {   
           ...
           /* socklen_t addr_size;
            char sndbuf[1024];
            char recvbuf[1024];
            snprintf(sndbuf, sizeof(sndbuf), "%s\r\n", "hello");
            read(osSocket, recvbuf, 1024);
            write(osSocket, sndbuf, 6);
            close(osSocket);*/
            
            theTask->Signal(kReadEvent);
#ifdef _EPOLL
            //theSocket->RequestEvent(EPOLLIN|EPOLLET);
#else
            theSocket->RequestEvent(EV_RE);
#endif   
            theTask->SetThreadPicker(Task::GetBlockingTaskThreadPicker()); //The Task processing threads
        }
