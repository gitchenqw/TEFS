...
static INT32 gvfs_query_querystr_parse(CHAR *input, CHAR *filehash,
	INT32 filehash_len)
{
	INT32 icnt               = 0;
	CHAR  *pfilehash         = NULL;
...
INT32 gvfs_ns_query_request(gvfs_http_request_t *r)
{
	gvfs_event_t      *rev;
	gvfs_connection_t *c;
	gvfs_upstream_t	  *nu;
	...
	rc = gvfs_query_querystr_parse(query_str, filehash, 64);
	if (GVFS_OK != rc) {
	...
	gvfs_log(LOG_INFO, "send query to ns len:%u id:%u type:%02x size:%lu success",
						36, sid, 0x0a, size);
