 ...
 *
 *
 * Version: write data to file queue 1.dat 2.dat 3.dat....(if(write > read) delete readfile)
 *
 */
...
int FileQueue::load_queue_head()
{
  ...
  {
    TBSYS_LOG(TFERROR, "create directoy : %s filed", queue_path_.c_str());
  ...
  {
    TBSYS_LOG(TFERROR, "open file: %s failed", path.c_str());
  ...
  TBSYS_LOG(TFDEBUG, "readno: %d, writeno: %d, queueSize: %d", queue_information_header_.read_seqno_,
...
int FileQueue::push(const void* const data, const int64_t len)
{
  ...
  {
    TBSYS_LOG(TFWARN, "invalid data: %p, length: %"PRI64_PREFIX"d", data, len);
  ...
  *((int32_t*) buffer) = size;
  ...
  if (ret != size)
  {
    TBSYS_LOG(TFWARN, "write filed, path: %s, fd: %d, lenght: %d, ret: %d, size: %d", queue_path_.c_str(), write_fd_,
        len, ret, size);
    ret = size - ret;
    if (ret > 0 && ret <= size)
    {
  ...
  TBSYS_LOG(TFDEBUG, "queue_size_: %d,size: %d,iret: %d, write_filesize_: %d, max_file_size: %u, write_seqno_: %d",
...
QueueItem *FileQueue::pop(int index)
{
  ...
  if (read_fd_ == -1)
  {
    TBSYS_LOG(TFWARN, "file %s:%d is close...", queue_path_.c_str(), queue_information_header_.read_seqno_);
    do
    {
      ++count;
      iret = open_read_file();
      if (iret != TFS_SUCCESS)
      {
        TBSYS_LOG(TFWARN, "reopen file %s: %d failed", queue_path_.c_str(), queue_information_header_.read_seqno_);
        ::close( read_fd_);
        read_fd_ = -1;
        continue;
      }
  ...
  while (true)
  {
    ...
    if (retsize < 0)
    {
      TBSYS_LOG(TFERROR, "read head size errors %d : %s from : %d", errno, strerror(errno), read_fd_);
      break;
    }
    if (retsize != sizeof(int32_t) && retsize != 0)
    {
      TBSYS_LOG(TFERROR, "read head size: %d errors %d : %s from : %d", retsize, errno, strerror(errno), read_fd_);
    ...
    if (retsize == sizeof(int32_t))
    {
      ...
      if (item == NULL)
      {
        TBSYS_LOG(TFERROR, "malloc failed...\n");
        break;
      }
...
int FileQueue::clear()
{
  ...
  {
    TBSYS_LOG(TFERROR, "file queue call clear error");
  }
  return TFS_ERROR;
}
...
int FileQueue::open_write_file()
{
  ...
  if (write_fd_ == -1)
  {
    TBSYS_LOG(TFERROR, "open file: %s failed", tmp);
    return TFS_ERROR;
  }
  return TFS_SUCCESS;
}
...
int FileQueue::open_read_file()
{
  ...
  if (iret < 0)
  {
    TBSYS_LOG(TFERROR, "call lseek filed: %s", strerror(errno));
    return TFS_ERROR;
  }
  return TFS_SUCCESS;
}
...
int FileQueue::write_header()
{
  ...
  if (iret == TFS_SUCCESS)
  {
    ...
    do
    {
      ...
      TBSYS_LOG(TFERROR, "write queue infromation header failed, path: %s, errors: %s", queue_path_.c_str(), strerror(
          errno));
      ::close( information_fd_);
      information_fd_ = -1;
      char tmp[MAX_PATH_LENGTH+1];
...
int FileQueue::recover_record()
{
  ...
  TBSYS_LOG(TFINFO, "%s: recoverRecord: %d", queue_path_.c_str(), count);
  memset(&queue_information_header_.pos_[0], 0, FILE_QUEUE_MAX_THREAD_SIZE * sizeof(unsettle));
  return write_header();
}
