...
namespace tfs
{
  namespace dataserver
  {
    ...
    int PhysicalBlock::pread_data(char* buf, const int32_t nbytes, const int32_t offset)
    {
      ...
      // converse relative data offset to absolute offset in block
    ...
    int PhysicalBlock::load_block_prefix()
    {
      ...
      TBSYS_LOG(TFDEBUG,
          "load block prefix. physical blockid: %u, fileid: %Lu,chunkid:%u prev physical blockid: %u, next physical blockid: %u, ret: %d",
          physical_block_id_, 
          block_prefix_.fileid,
          block_prefix_.chunkid,
          block_prefix_.prev_physic_blockid_,
    ...
    {
      TBSYS_LOG(TFDEBUG,"dump block prefix. fileid: %Lu,chunkid:%u prev physical blockid: %u, next physical blockid: %u",
          block_prefix_.fileid,
          block_prefix_.chunkid,
          block_prefix_.prev_physic_blockid_,
          block_prefix_.next_physic_blockid_);
    ...
    void PhysicalBlock::set_block_prefix(uint64_t fileid,uint32_t chunkid,const uint32_t prev_physic_blockid,
        const uint32_t next_physical_blockid)
    {
       block_prefix_.fileid = fileid;
      block_prefix_.chunkid = chunkid;
