  ...
  namespace common
  {
    ...
    public:
      ...
      inline void update_queue_information_header()
      {
        ...
        TBSYS_LOG(TFINFO, "Update QinfoHead(before): readSeqNo: %d, readOffset: %d, writeSeqNo: %d,"
        ...
        TBSYS_LOG(TFINFO, "Update QinfoHead(after): readSeqNo: %d, readOffset: %d, writeSeqNo: %d,"
          "writeFileSize: %d", head.read_seqno_, head.read_offset_, head.write_seqno_, head.write_filesize_);
      }
