 ...
 *
 *
 * Version: $Id: blockfile_manager.h 552 2011-06-24 08:44:50Z duanfei@taobao.com $
 *
 * Authors:
 ...
 *      - modify 2009-03-27
 *   blockfile manage,eg:create blockfile dir,load block file,load index
 */
  ...
  namespace dataserver
  {
    ...
    class BlockFileManager
    {
      ...
      public:
        ...
        int new_block(uint64_t fileid,uint32_t chunkid, uint32_t& physical_block_id, const BlockType block_type =
            C_MAIN_BLOCK);
        int new_ext_block(uint64_t fileid,uint32_t chunkid, const uint32_t physical_block_id,
            uint32_t& ext_physical_block_id, PhysicalBlock **tPhysicalBlock);
        int del_block(uint64_t fileid,uint32_t chunkid, const BlockType block_type = C_MAIN_BLOCK);

        LogicBlock* get_logic_block(uint64_t fileid,uint32_t chunkid, const BlockType block_type = C_MAIN_BLOCK);
        ...
        int get_logic_block_ids(common::VSTRING& logic_block_ids, const BlockType block_type = C_MAIN_BLOCK);
        ...
        int switch_compact_blk(uint64_t fileid,uint32_t chunkid);
        int expire_compact_blk(const time_t time, std::set<uint64_t>& erase_blocks);

        int set_error_bitmap(const std::set<uint32_t>& error_blocks);
        int reset_error_bitmap(const std::set<uint32_t>& reset_error_blocks);
        
        int get_bit_map_info(int32_t& bit_map_len, int32_t& normal_set_count,int32_t& error_set_count);
      ...
      private:
        ...
        LogicBlock* find_logic_block(uint64_t fileid, uint32_t chunkid, const BlockType block_type);
        int erase_logic_block(uint64_t fileid,uint32_t chunkid, const BlockType block_type);
    ...
    private:
        typedef std::map<uint64_t, std::map<uint32_t,LogicBlock*> > LogicBlockMap;
        typedef LogicBlockMap::iterator                  LogicBlockMapIter;
        typedef std::map<uint32_t,LogicBlock*>::iterator LogicChunkIter;
        typedef std::map<uint32_t, PhysicalBlock*> PhysicalBlockMap;
        typedef PhysicalBlockMap::iterator PhysicalBlockMapIter;
        
        LogicBlock* find_logic_block_map(LogicBlockMap& logicmap,uint64_t fileid,uint32_t chunkid);
        int insert_logic_block_map(LogicBlockMap& logicmap,LogicBlock* block,uint64_t fileid,uint32_t chunkid);
        int insert_logic_block_map(LogicBlockMap& logicmap,LogicBlock* block);
        int delete_logic_block_map(LogicBlockMap& logicmap,uint64_t fileid,uint32_t chunkid);
        uint64_t get_logic_block_size_map(const BlockType block_type);
