...
begin
    ...
    elseif opt=1 then          #备份数大于copies的块
        PREPARE sql2 FROM 'SELECT TBLOCK.FileID,TBLOCK.Blockindex,BlockInfo.DSID FROM (SELECT FileID,Blockindex,COUNT(*) AS num FROM BlockInfo WHERE Status=0 GROUP BY FileID,Blockindex) AS TBLOCK,BlockInfo WHERE TBLOCK.FileID=BlockInfo.FileID AND TBLOCK.Blockindex=BlockInfo.Blockindex AND num>? limit ?,?';
        SET @a=idx*@page_num;
        SET @b=@page_num;
        set @num=copies;
...
begin
    set @page_num = 50;
    PREPARE sql1 FROM 'SELECT TBLOCK.FileID,TBLOCK.Blockindex,BlockInfo.DSID FROM (SELECT FileID,Blockindex,COUNT(*) AS num FROM BlockInfo WHERE Status=0 GROUP BY FileID,Blockindex) AS TBLOCK,BlockInfo WHERE TBLOCK.FileID=BlockInfo.FileID AND TBLOCK.Blockindex=BlockInfo.Blockindex AND num<? limit ?,?';
    SET @a=idx*@page_num;
    SET @b=@page_num;
    set @num=copies;
