...
void EventContext::RequestEvent(int theMask)
{
    ...
    {
        fEventReq.ev_epoll.events = theMask;
#if MACOSXEVENTQUEUE
    ...
    { 
        ...
        fEventReq.ev_epoll.events = theMask;
        fEventReq.ev_epoll.data.fd = fFileDesc;
        fEventReq.ev_data = (void*)fUniqueID; 
        fEventReq.ev_type = EV_FD;
...
{
    struct epoll_req theCurrentEvent;
    ::memset( &theCurrentEvent, '\0', sizeof(theCurrentEvent) );
   
    while (true)
    {
        //qtss_printf("event thread is running\n");
        int theErrno = EINTR;
        while (theErrno == EINTR)
        {
            //qtss_printf("waiting loop is running\n");
#if MACOSXEVENTQUEUE
...
#endif
                theContext->ProcessEvent(theCurrentEvent.ev_epoll.events);
