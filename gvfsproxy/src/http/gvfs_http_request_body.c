...
static gvfs_int_t
gvfs_http_do_read_client_request_body(gvfs_http_request_t *r);

static gvfs_int_t
gvfs_http_write_request_body(gvfs_http_request_t *r);

static gvfs_int_t
gvfs_http_write_request_body_to_file(gvfs_http_request_t *r);


ssize_t gvfs_http_read_request_body(gvfs_http_request_t *r)
{
	ssize_t            n;
	gvfs_connection_t *c;
	gvfs_event_t      *rev;
...
void
gvfs_http_read_client_request_body_handler(gvfs_event_t *rev)
{
	gvfs_connection_t   *c;
	...
	gvfs_http_do_read_client_request_body(r);
...
gvfs_http_do_read_client_request_body(gvfs_http_request_t *r)
{
	...
	for ( ; ;) {
		...
		if (request_body->last == request_body->end) {
			...
			if (NULL != pc) {
				gvfs_http_write_request_body(r);
			}
			else {
				gvfs_http_write_request_body_to_file(r);
			}
			
			request_body->pos = request_body->last = request_body->start;
		}

		n = gvfs_http_read_request_body(r);
