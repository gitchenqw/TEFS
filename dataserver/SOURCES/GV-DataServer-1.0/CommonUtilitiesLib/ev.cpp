...
void select_startevents()
{
    ...
    sCookieArray = new void*[sizeof(fd_set) * 16];
    ::memset(sCookieArray, 0, sizeof(void *) * sizeof(fd_set) * 16);
...
#define FDCloseArraySize 1024
#define CookieArraySize  10240
static UInt32                    FDCloseArrayTop = 0;
static int                       EventPool[CookieArraySize];
static UInt32                    EventPoolSize = 0;          
static std::vector<epoll_event>  events_return(10);            //events epoll_wait return
static uint64_t                  maxevent;                   
static uint32_t    epoll_fd;                               //fd by epoll_create
static int        sNumFDsBackFromEpoll = 0;
static int        process_num = 0;                         //num of processed
static bool      isdone = false;

void epoll_startevents()
{
    memset(EventPool,-1,CookieArraySize);
    sFDsToCloseArray = new int[FDCloseArraySize];
    for (int i = 0; i < FDCloseArraySize; i++)
        sFDsToCloseArray[i] = -1;
    sCookieArray = new void*[CookieArraySize];
    ::memset(sCookieArray, 0, sizeof(void *) * CookieArraySize);
    if((epoll_fd = epoll_create(EPOLL_MAXSIZE)) < 0)
        qtss_printf("create epoll failed\n");
    int theErr = ::pipe((int*)&sPipes);
    Assert(theErr == 0);
    int PipeReadFlag = fcntl(sPipes[0], F_GETFL, 0);
    PipeReadFlag |= O_NONBLOCK;
    if (fcntl(sPipes[0], F_SETFL,PipeReadFlag) < 0)
    {
        perror(("set read pipe flaf failed!"));
        return;
    } 
    epoll_event ev;
    ...
    ev.data.fd = sPipes[0];
    //Add the read end of the pipe to the read mask
    if(epoll_ctl(epoll_fd, EPOLL_CTL_ADD,sPipes[0],&ev) < 0)
        qtss_printf("add read pipe to epoll failed\n");
    EventPool[sPipes[0]] = EPOLLIN|EPOLLET;
    EventPoolSize++;
}
...
{
    {
        ...
        OSMutexLocker locker(&sMaxFDPosMutex);
        //Clear this fd out of both sets
        sCookieArray[which] = NULL;
        if(epoll_ctl(epoll_fd,EPOLL_CTL_DEL,which,0)<0)
            qtss_printf("delete descriptor from epoll failed\n");
        EventPool[which] = -1; 
        EventPoolSize--;       
        //put this fd into the fd's to close array, so that when epoll wakes up, it will
        //close the fd
        Assert(FDCloseArrayTop < FDCloseArraySize);
        sFDsToCloseArray[FDCloseArrayTop++] = which;
    }
#if EV_DEBUGGING
    qtss_printf("removeevent: Disabled %d \n", which);
#endif
...
int  epoll_watchevent(struct epoll_req *req, int which)
{
    return epoll_modwatch(req,which);
}
int epoll_modwatch(struct epoll_req *req, int which)
{
    {
        OSMutexLocker locker(&sMaxFDPosMutex);
        //Add or remove this fd from the specified sets
    #if EV_DEBUGGING
            qtss_printf("modwatch: Enabling %d in readset\n", req->ev_epoll.data.fd);
    #endif
       if(EventPool[req->ev_epoll.data.fd]==-1)
       {
            if(epoll_ctl(epoll_fd,EPOLL_CTL_ADD,req->ev_epoll.data.fd, &req->ev_epoll) < 0)
                qtss_printf("addwatch: failed\n");
            EventPool[req->ev_epoll.data.fd] = which;
            EventPoolSize++;
       }
       else if(EventPool[req->ev_epoll.data.fd] != which)
       {
            if(epoll_ctl(epoll_fd,EPOLL_CTL_MOD,req->ev_epoll.data.fd, &req->ev_epoll) <0)
                qtss_printf("modwatch: failed\n");
       }   
        // Also, modifying the cookie is not preemptive safe. This must be
        // done atomically wrt setting the fd in the set. Otherwise, it is
        // possible to have a NULL cookie on a fd.
        Assert(req->ev_epoll.data.fd < CookieArraySize);
        Assert(req->ev_data != NULL);
        sCookieArray[req->ev_epoll.data.fd] = req->ev_data;
#if EV_DEBUGGING
        qtss_printf("modwatch: active num=%lu\n",EventPoolSize);
#endif
    }
    //write to the pipe so that select wakes up and registers the new mask
    int theErr = ::write(sPipes[1], "p", 1);
#if EV_DEBUGGING
        qtss_printf("modwatch:write 1 bytes to pipe\n");
#endif
    Assert(theErr == 1);
    return 0;
}
...
{
    {
        //Check the fds to close array, and if there are any in it, close those descriptors
        OSMutexLocker locker(&sMaxFDPosMutex);
        for (UInt32 theIndex = 0; theIndex < FDCloseArrayTop; theIndex++)
        {
            (void)::close(sFDsToCloseArray[theIndex]);
        }
        FDCloseArrayTop = 0;
    ...
    do
    {
        len = read(sPipes[0], &theBuffer[0], 4192);
    }while(len > 0);
    if(sNumFDsBackFromEpoll==0)
        return true;
    if(sNumFDsBackFromEpoll==1&&events_return[0].data.fd == sPipes[0])
        return true;
    return false;
}

int epoll_waitevent(struct epoll_req *req, void* )//onlyForMacOSX
{
    
    while(noevent()||isdone)
    {
        //Periodically time out the select call just in case we
        //are deaf for some reason
        // on platforw's where our threading is non-preemptive, just poll select
        ...
        {
            OSMutexLocker locker(&sMaxFDPosMutex);
            maxevent = EventPoolSize;
        }
        if(maxevent==0)
            maxevent = 10;
        events_return.resize(maxevent);
        sNumFDsBackFromEpoll = epoll_wait(epoll_fd,&events_return[0],maxevent,tv);

#if EV_DEBUGGING
        qtss_printf("waitevent: back from epoll. Result = %d,error:%d,maxevent=%lu\n", sNumFDsBackFromEpoll,errno,maxevent);
#endif
        isdone = false;
    }
    if(process_num < sNumFDsBackFromEpoll)
    {
        memcpy(&req->ev_epoll,&events_return[process_num++],sizeof(epoll_event));
           Assert(req->ev_epoll.data.fd < CookieArraySize);
        req->ev_data = sCookieArray[req->ev_epoll.data.fd];
        return 0;
    }
    else
    {
